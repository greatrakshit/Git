...
public class AddContactsList extends Activity implements OnItemClickListener{
	...
	private static final int MAX_CONTACTS_ALLOWED = 5;
	private static final int MIN_CONTACTS_ALLOWED = 2;
	DBAdapter mDb;
	...
	public void deleteSelectedContacts(View v){
		...
		int remaining_contacts = (list.size())-contact.length;
		Log.d(getClass().getSimpleName(), "remaining_contacts :"+remaining_contacts);
		if(remaining_contacts >= MIN_CONTACTS_ALLOWED){
			...
			Toast.makeText(getApplicationContext(), getResources().getString(R.string.two_contacts_mandatory), Toast.LENGTH_LONG).show();
		}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		Cursor c = mDb.fetchContacts();
		if(c.getCount() > MIN_CONTACTS_ALLOWED){
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.delete_contact:
			if(list.size() > (MIN_CONTACTS_ALLOWED+1))
				toggleDeleteContactsVisibility();
			else
				Toast.makeText(getApplicationContext(), getResources().getString(R.string.two_contacts_mandatory), Toast.LENGTH_LONG).show();
			break;
